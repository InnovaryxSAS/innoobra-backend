...
public class CreateAttributeHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {

    private static final Logger logger = LoggingHelper.getLogger(CreateAttributeHandler.class);
    ...
    public CreateAttributeHandler(AttributeService attributeService) {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        ...
        try {
            ...
            CreateAttributeRequestDTO requestDTO = OBJECT_MAPPER.readValue(input.getBody(), CreateAttributeRequestDTO.class);
            
            ValidationHelper.validateAndThrow(requestDTO, ValidationGroups.Create.class);
            
            Attribute attribute = DTOMapper.toAttribute(requestDTO);
            
            Attribute createdAttribute = attributeService.createAttribute(attribute);
            
            AttributeResponseDTO responseDTO = DTOMapper.toResponseDTO(createdAttribute);
            ...
            LoggingHelper.logEntityAlreadyExists(logger, "Attribute", e.getMessage());
